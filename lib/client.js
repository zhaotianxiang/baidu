
'use strict';

const fs = require('fs');
const moment = require('moment');
const Client = require('floodesh/client');

class baidu extends Client{
	constructor(){
		super();
		this.name = 'baidu';
		//this.seed = ['https://www.google.com/'];
		this.once('init', (done) => this.onInit(done))
			.on('data', (data, done) => this.onData(data, done))
			.once('exit', () => this.onEnd());
	}

	onInit(done){
		this.output = fs.createWriteStream('/data/baidu/baidu_'+moment().format('YYYY-MM-DD')+'.csv');
		this.output.write('\ufeff');
		done();
	}
	
	initRequests(){// will not be invoked if seed is not empty.
		return [{opt:'https://www.google.com/', next:'home'},{opt:'http://www.amazon.com/', next:'home'}];
	}
	
	onData(data,done){
		this.output.write(data.get('data'));
		done();
	}
	
	onComplete(tasks){
		return tasks.length;
	}
	
	onEnd(){
		this.output.end();
	}
}

module.exports = baidu;
